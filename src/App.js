...
class App extends Component {
  state = {
    ...
    score: 0,
    time: 60,
    matchedImageUrl: imagesList[0].imageUrl,
    category: 'FRUIT',
  }
  ...
  onClickThumbnailImage = thumbnailUrl => {
    const {matchedImageUrl} = this.state
    const matchedImgObj = imagesList.filter(
      eachImage => eachImage.thumbnailUrl === thumbnailUrl,
    )

    const {imageUrl} = matchedImgObj[0]

    if (imageUrl === matchedImageUrl) {
      const newImgUrl =
        imagesList[Math.floor(Math.random() * imagesList.length)].imageUrl
      this.setState(prevState => ({
        matchedImageUrl: newImgUrl,
  ...
  render() {
    const {isTimerRunning, score, time, matchedImageUrl, category} = this.state
    const filteredThumbnailsImages = imagesList.filter(
      ...
      <div className="app-conainer">
        <div className="content-container">
          <nav className="nav-bar">
            ...
            />
            <ul className="score-and-stopwatch-container">
              <li className="score">
                <p>
                  Score:<span className="numbering"> {score}</span>
                </p>
              </li>
              <li className="timer-and-logo-container">
                <img
          ...
          {isTimerRunning && (
            <div className="image-tabs-thumbnails-container">
              <img src={matchedImageUrl} alt="match" className="match-image" />
              ...
              <ul className="tab-list">
                {tabsList.map(eachTab => (
                  <li className="tab-item" key={eachTab.tabId}>
                    <button
                      className={`tab-btn ${
                        category === eachTab.tabId ? 'active-tab' : ''
                      }`}
                      type="button"
              ...
              <ul className="thumbnails-list">
                {filteredThumbnailsImages.map(eachimage => (
                  <li
                  ...
                  >
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        this.onClickThumbnailImage(eachimage.thumbnailUrl)
                      }
                    >
                      <img
